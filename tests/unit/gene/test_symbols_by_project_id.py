...
def test_symbols_by_geneset_id(geneset_gene_symbols):
    ...
    result = symbols_by_project_id(cursor, 1)
...
def test_symbols_by_project_id_execute_raises_error(all_psycopg_errors):
    ...
    with pytest.raises(all_psycopg_errors, match="Error message"):
        symbols_by_project_id(cursor, 1)

    assert cursor.execute.call_count == 1
    assert cursor.fetchall.call_count == 0


def test_symbols_by_project_id_fetchall_raises_error(all_psycopg_errors):
