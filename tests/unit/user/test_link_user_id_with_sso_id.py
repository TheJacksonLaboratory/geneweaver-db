...
def test_link_user_id_with_sso_id(sso_id_exists_return, cursor) -> None:
    ...
    with patch('geneweaver.db.user.sso_id_exists',
               return_value=sso_id_exists_return) as sso_id_exists:
        ...
        if sso_id_exists_return is False:
            ...
            with pytest.raises(ValueError, match="SSO ID is already linked to a different account"):
                link_user_id_with_sso_id(cursor, 1, 'auth0|5f9b7b7a9a9a9a9a9a9a9a9a')
...
def test_link_user_id_with_sso_id_execute_raises_error(sso_id_exists, all_psycopg_errors, cursor_execute_raises_error) -> None:
...
@patch('geneweaver.db.user.sso_id_exists', return_value=False)
def test_link_user_id_with_sso_id_fetchone_raises_error(sso_id_exists, all_psycopg_errors, cursor_fetchone_raises_error) -> None:
    with pytest.raises(all_psycopg_errors, match="Error message"):
